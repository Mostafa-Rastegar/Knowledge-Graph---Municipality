...
سپس سامانه روی **کل ۵۰۰ سند بخش Dev** اجرا شد و **F1 = ۲۸٫۶۴** گرفت. اختلاف با عدد زیرمجموعه کمتر از یک واحد است، پس نتیجه به یک نمونه‌ی کوچک وابسته نیست.

در مرحله‌ی سوم، دیتاست به **پیکره‌ی کامل DocRED** گسترش یافت: ۱۰۶٬۹۲۴ سند و ۱٫۵۶ میلیون سه‌تایی برچسب‌خورده، ۲۶ برابر بزرگ‌تر از مرحله‌ی قبل. برای این مقیاس، استخراج هم‌زمان (۴۸ درخواست موازی)، ازسرگیری بدون تکرار کار، ارزیاب جریانی و بارگذار دسته‌ای Neo4j ساخته شد. روی ۹۹۸ سند بخش Dev با برچسب‌های اصلی DocRED، F1 برابر **۲۱٫۳۸** شد. همان پیش‌بینی‌ها روی ۴۹۸ سند مشترک، با برچسب‌های اصلی ۲۱٫۰۳ و با برچسب‌های بازبینی‌شده **۲۶٫۶۴** می‌گیرند؛ دقت از ۱۶٫۸ به ۳۵٫۶ دو برابر می‌شود. این اندازه‌گیری مستقیمِ اثر منفی کاذب در برچسب‌های اصلی است که مقاله‌ی Re-DocRED به آن اشاره می‌کند. در پایان همان مدل بدون تغییر معماری روی اسناد فارسی شهرداری اجرا شد و به‌عنوان مطالعه‌ی موردی ایرانی گزارش شد.
...
کار در دو مرحله روی این داده انجام شد. مرحله‌ی نخست روی Re-DocRED بود: همان ۴٬۰۵۳ سندِ برچسب‌خورده‌ی انسانی DocRED، با برچسب‌هایی که بازبینی و کامل شده‌اند. مرحله‌ی دوم کل پیکره‌ی DocRED را برداشت که ۲۶ برابر بزرگ‌تر است و بخش عمده‌اش با هم‌ترازی Wikidata برچسب خورده (نظارت‌ازدور).

### ۵.۰ جدول ۰ — پیکره‌ی کامل DocRED

اعداد با `python -m src.docred stats` از روی فایل‌های Parquet محاسبه شد و در `data/benchmark/docred/stats.json` ذخیره است.
...
دو نکته درباره‌ی برچسب‌ها. نخست، برچسب‌های اصلی DocRED ناقص‌اند: مقاله‌ی Re-DocRED نشان داد بسیاری از رابطه‌های موجود در متن برچسب نخورده‌اند. بخش ۷٫۸ این ناقص بودن را با همان پیش‌بینی‌ها و دو مجموعه برچسب اندازه می‌گیرد. دوم، برچسب‌های بخش نظارت‌ازدور از هم‌ترازی خودکار با Wikidata می‌آیند، نه از انسان؛ روی این بخش «توافق با برچسب ازدور» گزارش می‌شود، نه F1.
...
اجرای ۵۰۰ سند تک‌نخی حدود هفت ساعت طول کشیده بود. با همان سرعت، پیکره‌ی کامل بیش از یک ماه می‌گرفت. چهار تغییر این را ممکن کرد و هر چهار مورد روی داده‌ی واقعی اندازه‌گیری شد.
...
مثال‌های پرامپت هفت برابر ورودی را بزرگ می‌کنند، ولی چون پیشوند ثابت است، ۸۳٪ آن از کش سرویس می‌آید. توکن خروجی، نه ورودی، سهم اصلی هزینه است: هر رابطه با جمله‌ی شاهدش حدود ۴۰ توکن است. با قیمت فهرست مدل، گذر A حدود ۰٫۰۰۲۴ دلار و گذر B حدود ۰٫۰۰۳۷ دلار بر سند تمام می‌شود؛ کل پیکره در گذر A حدود ۲۵۰ دلار.
...
**دو هشدار برای خواندن این جدول.** نخست، سطر GPT-3.5 واقعاً zero-shot است ولی سطرهای ما نیستند؛ مقایسه‌ی مستقیم به سود ما است. سطر منصفانه برای مقایسه‌ی zero-shot، نسخه‌ی v1 با F1 = ۱۶٫۸۸ است که آن هم هستان‌شناسی مشتق از Train دارد. دوم، سه سطر نخست ما روی بخش Dev است و تنها سطر آخر (۳۱٫۷۰) روی همان بخش Test بازبینی‌شده‌ای است که مدل‌های نظارت‌شده روی آن گزارش شده‌اند.
...
در این مرحله سه چیز عوض شد و بقیه‌ی پیکربندی **قفل** ماند (همان هستان‌شناسی، همان دو مثال، همان سه قاعده):

۱. مدل زبانی از `openai/gpt-4.1-mini` به مدل `sonnet` (کلاد، از راه Claude Code CLI با `effort=low`) تغییر کرد.
۲. قالب پاسخ **فشرده** شد: مدل به‌جای JSON کامل، شماره‌ی موجودیت و شماره‌ی جمله را برمی‌گرداند. این کار طول پاسخ را حدود چهار برابر کم می‌کند.
۳. ترکیب دو گذر از **اجتماع** به **اشتراک** (اجماع) تغییر کرد: فقط سه‌تایی‌هایی می‌مانند که هر دو گذر پیدا کرده‌اند.
...
نقش دو گذر برعکس مرحله‌ی قبل شد. با مدل تازه، مثال دیگر دقت نمی‌خرد بلکه بازخوانی می‌خرد (۳۶٫۵۳ ← ۴۱٫۷۹)، و این **اشتراک** دو گذر است که دقت را بالا می‌برد (۳۲٫۵۷ ← ۳۹٫۰۸). اشتراک، خروجی را از ۱۶٬۷۴۱ به ۱۰٬۶۱۶ سه‌تایی کوچک می‌کند ولی F1 را بالا می‌برد، چون سه‌تایی‌هایی که فقط یک گذر دیده بیشترشان غلط‌اند.
...
برچسب‌های بازبینی‌شده ۲٫۸ برابر بیشترند و دقت اندازه‌گیری‌شده‌ی همان پیش‌بینی‌ها از ۴۰٫۲۰ به **۶۶٫۴۲** می‌رسد. یعنی حدود ۴۰٪ از «مثبت‌های کاذب» روی برچسب اصلی، رابطه‌هایی هستند که در متن وجود دارند و فقط برچسب نخورده‌اند. این عدد، همان ادعای مقاله‌ی Re-DocRED است که اینجا با سامانه‌ی ما مستقیماً اندازه‌گیری شد. نتیجه‌ی عملی برای خواندن جدول‌های این گزارش: هر عدد دقت روی برچسب اصلی DocRED یک **کران پایین** است. بازخوانی برعکس عمل می‌کند، چون مخرج آن ۲٫۸ برابر بزرگ‌تر شده است.
...
۱. عدد بخش Train برچسب‌خورده (۳۶٫۳۳) تقریباً برابر عدد بخش Dev (۳۶٫۲۵) است، با اینکه اندازه‌ی آن سه برابر است. یعنی سامانه روی بخش Dev بیش‌برازش نشده است. این نزدیک‌ترین چیزی است که بدون برچسب Test می‌شود به یک ارزیابی بی‌طرف رسید.
۲. برچسب‌های DocRED برای بخش Test عمومی نیستند، پس برای آن ۱٬۰۰۰ سند فقط تعداد سه‌تایی گزارش می‌شود، نه دقت.
۳. Ign F1 برای بخش Train معنا ندارد و صفر گزارش می‌شود، چون کلید «دیده‌شده در Train» از خود همان بخش ساخته می‌شود.

**۷.۸.۴ بخش نظارت‌ازدور — نمونه‌ی ۲۰ هزار سندی.** بخش نظارت‌ازدور ۱۰۱٬۸۷۳ سند دارد و برچسب آن ماشینی و نویزی است. یک نمونه‌ی تصادفی ۲۰٬۰۰۰ سندی (بذر ۴۲) انتخاب شد و اجرا با گذر A روی **۱۰٬۹۸۵ سند** پیش رفت تا سهمیه‌ی هفتگی مدل تمام شد. اجرا ازسرگیری‌پذیر است و همین ۱۰٬۹۸۵ سند برای نشان دادن مقیاس کافی است:
...
**این اعداد «توافق» هستند، نه دقت.** برچسب طلایی این بخش (۱۶۰٬۵۵۵ سه‌تایی روی این ۱۰٬۹۸۵ سند) خودش خروجی یک سامانه‌ی خودکار است، پس اختلاف می‌تواند خطای هر دو طرف باشد. عدد پایین‌تر از بخش Dev انتظار درستی است.

لایه‌ی بستار در این مقیاس ۳۶٬۷۹۹ سه‌تایی تازه ساخت: قاعده‌ی وارون ۲۶٬۵۳۱، قاعده‌ی تراگذری ۶٬۴۸۶ و قاعده‌ی زنجیره‌ی کشور ۳٬۷۸۲. این لایه بازخوانی را ۳٫۴ واحد بالا برد و دقت را ۲٫۳ واحد پایین آورد؛ برایند روی F1 مثبت ولی کوچک است (۲۶٫۱۳ ← ۲۶٫۴۵).
...
توان عملیاتی پایدار روی بخش نظارت‌ازدور **۴۹٫۳ سند در دقیقه** بود. گلوگاه پردازنده است، نه حافظه: هر کارگر حدود ۱۷۰ مگابایت حافظه می‌گیرد. کش پرامپت در گذر B کار می‌کند (۲۰٫۱ میلیون توکن از ۴۳٫۷ میلیون توکن ورودی از کش آمد)، چون مثال‌ها در ابتدای پرامپت ثابت‌اند.
...
کشیدن ۲۵۳ هزار یال با هم معنا ندارد، پس رابط وب یک جست‌وجوی موجودیت گرفت: نام را می‌نویسید و همسایگی آن با شاهد هر یال می‌آید. نمونه‌ی زیر همسایگی `Tehran` است: ۵۸ موجودیت و ۹۲ رابطه. پنل سمت راست جمله‌ی منبع هر رابطه را نشان می‌دهد و سربرگ، اندازه‌ی کل پایگاه را.
...
۱. برچسب بخش Test در DocRED عمومی نیست، پس هیچ عدد دقتی روی آن ۱٬۰۰۰ سند گزارش نمی‌شود. نزدیک‌ترین ارزیابی بی‌طرف، بخش Train برچسب‌خورده (۳٬۰۵۳ سند، F1 = ۳۶٫۳۳) و بخش Test بازبینی‌شده‌ی Re-DocRED (۵۰۰ سند، F1 = ۳۱٫۷۰) است (بخش ۷٫۸٫۳).
۶. برچسب‌های اصلی DocRED ناقص‌اند و دقت را کمتر از واقع نشان می‌دهند؛ روی همان ۴۹۸ سند، دقت با برچسب بازبینی‌شده از ۴۰٫۲۰ به ۶۶٫۴۲ می‌رسد (بخش ۷٫۸٫۲). برچسب بخش نظارت‌ازدور از این هم نویزی‌تر است؛ عدد آن بخش «توافق» است، نه دقت.
